...
# 4. Build RNN model
model = Sequential([
    SimpleRNN(32, return_sequences=True, input_shape=(None, vocab_size)),
...
# 6. Predict next word for each input word
output = model.predict(X, verbose=0)
pred_indices = np.argmax(output, axis=2)[0]
predicted_words = [index_word.get(i, '?') for i in pred_indices]
